...
public class MiniProject1 extends Applet implements Runnable,MouseListener,MouseMotionListener,ActionListener
...
int typeOfCandiestoConstruct[]={1,1,11,11,31,31,21,31,2,3},tt=0;
...
g.drawString(String.valueOf(timer.getTime()),250,650);
...
gridreturn=candyCrusher.fillEmptySpaces(g,Grid,typeOfCandiestoConstruct,10);
...
gridreturn=candyCrusher.fillEmptySpaces(g,Grid,typeOfCandiestoConstruct,5);

Grid=gridreturn.getNewGrid();

candyCrusher.displayGrid(g,gridreturn.getOldGrid(),Grid,0);

Grid[gameStatus%10][gameStatus%10][1]=5;

}

 

else if(mouseRelease || candyCrushState==1)
...
GridReturn gg = candyCrusher.performCandyCrush(g, Grid);

candyCrusher.displayGrid(g,Grid,Grid,0);
...
if(crushCandies==true)

{

if(candyCrushState==0)
...
g.setFont(myFont);
	  ...
	  Color c2=new Color(255,20,147);
	  Color c1=new Color(255,0,255);
	  if(p==0)
	  {
	  g.setColor(c2);
	  g.drawRoundRect(200,350,120,80,20,20);
	  g.fillRoundRect(200,350,120,80,20,20);
	  g.setColor(Color.black);
	  g.drawRoundRect(200,350,120,80,20,20);
	  g.drawString("Play",215,398);
	  }
	  else if(p==1)
	  {
		  g.setColor(c1);
...
else if(candyCrushState==1)

{

gridreturn=candyCrusher.fillEmptySpaces(g,gridreturn.getNewGrid(),typeOfCandiestoConstruct,10);

candyCrusher.displayGrid(g,gridreturn.getNewGrid(),gridreturn.getOldGrid(),0);

if(gridreturn.gridsDifferent()==false)
...
while(gridreturn.getSpecialCandies().size()!=0)
...
g.drawString(String.valueOf(tt),100,100);
